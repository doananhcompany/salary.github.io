...
productSelectElement.innerHTML = returnSelectTemplate(dataArr);

// display paid
addDot = (element, isMoney) => {
  const idErrElement = element.id === 'paid' ? 'paidErr' : 'weightErr';
...
justNumber = (element, maxLength) => {
  const idErrElement = element.id === 'paid' ? 'paidErr' : 'weightErr';

  element.addEventListener('keydown',function (event) {
      allowValue = ['1','2','3','4','5','6','7','8','9','0','Backspace'];
      isAlreadyFormat = false;
      if((element.value.length > maxLength && event.key !== 'Backspace')) {
          showAndHideErr(idErrElement, true, 'Số ký tự đã tối đa')
          return event.preventDefault();
      }
      
      if(!allowValue.includes(event.key)){
          if(!allowValue.includes(event.key)) {
...
isFormValid = () => {
 ...
 return productSelectElement.value && isweightValueValid;
}


addDot(paidElement,true)
addDot(weightElement,false)
justNumber(paidElement,12)
justNumber(weightElement,7)
...
displayData = (DATAFORTABLE) => {
  ...
  DATAFORTABLE.map((item) => {
      ...
      <tr class="result-row">
          ...
          <td></td>
          <td></td>
          <td class="result-product">${item.product}</td>
          <td class="result-weight">${item.weight} kg</td>
          ...
          <td class="result-del"><i class="fa-solid fa-trash-can" onclick="deleteProduct(${item.result_id})"></i></td>
      </tr>`
  })
  ...
  <tr class="result-row-fixed">
      ...
      <td></td>
      <td></td>
      <td class="result-product"></td>
      <td class="result-weight"></td>
      <td class="result-store"></td>
      <td class="result-money align-right money"><h3>${formatCurrentcy(totalPaid)} đ</h3></td>
      <td class="result-money align-right money"><h3>${formatCurrentcy(totalBonus.toFixed())} đ</h3></td>
      <td class="result-money align-right money"><h3>${formatCurrentcy(totalDebt)} đ</h3></td>
      <td class="result-money align-right money"><h3>${formatCurrentcy(totalRevenue)} đ</h3></td>
...
document.getElementById('add').addEventListener('click', addDataToTable)
